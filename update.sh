...
if command -v composer &>/dev/null; then
  ...
  composer_self_update_cmd=(composer self-update)

  # If Composer binary is system-owned (e.g. /usr/local/bin/composer root:root),
  # self-update needs elevated permissions.
  if [[ "$DRY_RUN" -ne 1 && ! -w "$composer_bin" ]]; then
    if sudo -n test -w "$composer_bin"; then
      log "Composer binary is not writable by current user; retrying self-update with sudo"
      composer_self_update_cmd=(sudo composer self-update)
    else
      log_warning "Composer binary is not writable and sudo cannot write it: $composer_bin"
      mark_result "Composer" "FAILED"
    fi
  fi

  if [[ "${UPDATE_RESULTS["Composer"]:-}" != "FAILED" ]]; then
    if run_step "Composer self-update" "${composer_self_update_cmd[@]}"; then
